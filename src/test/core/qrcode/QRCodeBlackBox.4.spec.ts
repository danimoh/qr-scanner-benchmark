...
export default class QRCodeBlackBox4Spec extends AbstractBlackBoxSpec {
    ...
    public constructor(reader: ImageReader) {
        super('src/test/resources/blackbox/qrcode-4', reader);
...
describe('QRCodeBlackBox.4 zxing', () => {
    it('testBlackBox 4 zxing', done => {
        let start = Date.now();
        const test = new QRCodeBlackBox4Spec(new QrReaderZXing());
...
describe('QRCodeBlackBox.4 cozmo', () => {
...
describe('QRCodeBlackBox.4 lazlo', () => {
    it('testBlackBox 4 lazlo', done => {
        let start = Date.now();
        const test = new QRCodeBlackBox4Spec(new QrReaderLazlo());
        return test.testBlackBox(() => {
            console.log(`Took ${(Date.now() - start) / 1000}s`);
            done();
        });
